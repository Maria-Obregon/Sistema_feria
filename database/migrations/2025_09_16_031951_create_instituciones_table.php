   ...
   public function up(): void
    {
        Schema::create('instituciones', function (Blueprint $table) {
            ...
            $table->string('modalidad', 120); // ahora VARCHAR para usar catálogo
            $table->string('codigo_presupuestario', 20)->unique();

            // FKs (del diagrama)
            $table->foreignId('direccionreg_id')->constrained('regionales')->restrictOnDelete();
            $table->foreignId('circuito_id')->constrained('circuitos')->restrictOnDelete();

            // CAMBIO: de enum(...) -> string
            $table->string('tipo', 120); // ahora VARCHAR para usar catálogo
